...
async function getBlogPost(slug: string) {
  const response = await fetch(`/api/blog/${slug}`, { next: { revalidate } })
          ...
          <div className="mx-auto max-w-3xl">
            ...
            <div className="mt-10 rounded-lg border bg-card p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold">{post.authors?.name}</h3>
                  <p className="text-sm text-muted-foreground">{post.authors?.bio || "ACM Member"}</p>
                </div>
              </div>
            </div>
                  ...
                  {relatedPosts.map((relatedPost) => (
                    <Card key={relatedPost.id} className="overflow-hidden">
                      <CardContent className="p-6">
                        ...
                        <p className="mb-4 text-muted-foreground">{relatedPost.excerpt}</p>
