...
const TreasureHunt: React.FC = () => {
	...
	return (
		<div
			className='flex flex-col items-center rounded p-6 font-sans relative overflow-hidden'
			style={{
				background: `linear-gradient(120deg, #FF6B6B 0%, #FF9E80 100%)`,
				backgroundSize: '400% 400%',
				animation: 'gradientAnimation 15s ease infinite',
			}}
		>
			{/* Animated background elements with tropical theme */}
			<div className='absolute inset-0 overflow-hidden'>
				{/* Floating particles resembling palm leaves */}
				{[...Array(20)].map((_, i) => (
					<div
						key={i}
						className='absolute rounded-full opacity-20'
						style={{
							...
							left: `${(i * 5) % 100}%`,
							transform: `translate(${
								Math.sin(i + bgPosition.x / 100) * 30
							}px, ${Math.cos(i + bgPosition.y / 100) * 30}px)`,
							transition: 'transform 0.5s ease-out',
						}}
					/>
				))}

				{/* Animated stars as music notes */}
				{[...Array(30)].map((_, i) => (
					<div
						key={`star-${i}`}
				...
				{/* Animated glow effects resembling sun */}
				<div className='absolute top-1/4 left-1/4 w-1/2 h-1/2 rounded-full bg-yellow-400 opacity-10 blur-3xl animate-pulse'></div>
				<div
					className='absolute bottom-1/4 right-1/4 w-1/3 h-1/3 rounded-full bg-teal-500 opacity-10 blur-3xl animate-pulse'
					style={{ animationDelay: '1s' }}
				></div>
			</div>

			{/* Logo Image - Styled as a sun */}
			<div className='mt-8 mb-6 z-10'>
				<div className='w-32 h-32 bg-yellow-400 rounded-full shadow-xl overflow-hidden border-4 border-teal-300 p-2'>
					<img
						src='/pearl-logo.png?height=128&width=128'
						alt='Neon Party Logo'
						className='w-full h-full object-cover rounded-full'
					/>
				</div>
			...
			<div className='mb-10 text-center z-10'>
				...
				<div className='h-1 w-32 bg-teal-400 mx-auto mt-2 rounded-full'></div>
			</div>

			{/* Main Content Container */}
			<div className='bg-white bg-opacity-90 backdrop-filter backdrop-blur-sm w-full max-w-md rounded-2xl shadow-2xl p-8 transition-all duration-500 hover:shadow-teal-400/50 z-10 border border-teal-200'>
				{/* Riddle Section */}
				<div className='mb-8 text-center'>
					<div className='inline-block bg-teal-500 text-white px-4 py-2 rounded-full font-bold mb-4 transform -rotate-2 shadow-md'>
						Party Riddle #1
					</div>
					<p className='text-gray-700 italic text-lg leading-relaxed'>
				...
				<form onSubmit={handleSubmit} className='mb-6'>
					<div className='mb-4 relative'>
						<input
							...
							onChange={(e) => setAnswer(e.target.value)}
							placeholder='Enter your answer...'
							className='w-full px-6 py-3 rounded-full border-2 border-teal-300 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent text-lg transition-all duration-300'
							required
						/>
					</div>
					<button
						type='submit'
						className='w-full bg-gradient-to-r from-yellow-500 to-teal-500 hover:from-yellow-600 hover:to-teal-600 text-white py-3 px-6 rounded-full text-lg font-bold shadow-lg transition-all duration-300 transform hover:scale-105 hover:rotate-1 active:scale-95'
					>
						Join The Party
					</button>
				</form>
				...
				<div className='text-center'>
					<button
						onClick={() => setShowModal(true)}
						className='text-teal-600 hover:text-yellow-600 font-medium transition-colors duration-300 text-lg flex items-center justify-center mx-auto'
					>
						<span className='mr-2'>🌴</span> Party Rules
					</button>
				</div>
			</div>
			...
			{showModal && (
				<div className='fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50'>
					<div className='bg-white rounded-2xl shadow-2xl p-6 max-w-md w-full animate-float-in relative overflow-hidden'>
						<div className='absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-yellow-500 to-teal-500'></div>
						<div className='flex justify-between items-center mb-6'>
						...
						<div className='mb-6 text-gray-700'>
							<ol className='space-y-3'>
								<li className='flex items-start'>
									<span className='bg-yellow-100 text-yellow-600 rounded-full w-6 h-6 flex items-center justify-center mr-2 font-bold'>
										1
									</span>
									<span>
										Read the riddle carefully and ponder its
										meaning.
									</span>
								</li>
								<li className='flex items-start'>
									...
									<span>
										Type your answer in the party box.
									</span>
								</li>
								<li className='flex items-start'>
									<span className='bg-yellow-100 text-yellow-600 rounded-full w-6 h-6 flex items-center justify-center mr-2 font-bold'>
									...
									<span>
										Each party challenge must be solved to
										proceed.
									</span>
								</li>
								<li className='flex items-start'>
									...
									<span>
										Correct answers will unlock the next
										event in your journey.
									</span>
								</li>
								<li className='flex items-start'>
									<span className='bg-yellow-100 text-yellow-600 rounded-full w-6 h-6 flex items-center justify-center mr-2 font-bold'>
									...
									<span>
										The real party begins at the end of your
										quest!
									</span>
								</li>
							</ol>
						...
						<button
							onClick={() => setShowModal(false)}
							className='w-full bg-gradient-to-r from-yellow-500 to-teal-500 hover:from-yellow-600 hover:to-teal-600 text-white py-3 px-4 rounded-full font-bold transition-colors duration-300 flex items-center justify-center'
						>
							<span className='mr-2'>🎵</span> Back to the Party
						</button>
					</div>
				</div>
			)}
				...
				<div className='fixed bottom-4 right-4 bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-lg shadow-lg animate-shake z-50'>
					<div className='flex items-center'>
						...
						<p className='font-medium'>
							Wrong answer! The party continues elsewhere...
						</p>
					</div>
				</div>
			)}
